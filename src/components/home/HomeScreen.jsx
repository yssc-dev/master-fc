  ...
  return (
    <div style={hs.container}>
      ...
      {teamNames.map(teamName => {
        ...
        return (
          <div key={teamName} style={{ ...hs.card, border: isSelected ? "1px solid #22d3ee" : "1px solid #334155" }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {isSelected && <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: "#22d3ee22", color: "#22d3ee", fontWeight: 600 }}>현재</span>}
              </div>
      ...
      {/* 팀 추가 */}
      <div style={{ ...hs.card, cursor: "pointer", border: "1px dashed #475569", background: "transparent", textAlign: "center" }}
        onClick={() => setShowAddInfo(true)}
        onMouseEnter={e => e.currentTarget.style.borderColor = "#22d3ee"}
        onMouseLeave={e => e.currentTarget.style.borderColor = "#475569"}>
        <div style={{ fontSize: 20, marginBottom: 4 }}>+</div>
        <div style={{ fontSize: 14, fontWeight: 600, color: "#94a3b8" }}>팀 추가하기</div>
      </div>

      {showAddInfo && (
        <div style={{ width: "100%", maxWidth: 340, background: "#1e293b", borderRadius: 12, padding: 16, marginBottom: 12, border: "1px solid #334155" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#f8fafc", marginBottom: 8 }}>팀 추가 방법</div>
          <div style={{ fontSize: 13, color: "#94a3b8", lineHeight: 1.6 }}>
            관리자에게 <b style={{ color: "#22d3ee" }}>회원인증 시트</b>에 등록을 요청하세요.
          </div>
          <div style={{ fontSize: 12, color: "#64748b", marginTop: 8, lineHeight: 1.5 }}>
            시트에 팀이름, 이름, 휴대폰뒷자리를 등록하면 다음 로그인 시 자동으로 팀 목록에 나타납니다.
          </div>
          <button onClick={() => setShowAddInfo(false)} style={{ background: "#334155", color: "#94a3b8", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 12, fontWeight: 600, cursor: "pointer", marginTop: 10, width: "100%" }}>
            닫기
          </button>
        </div>
      )}

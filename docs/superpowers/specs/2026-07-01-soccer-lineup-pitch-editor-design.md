...
| 피치 동작 | 의미 | 리듀서 op | 이벤트 |
|---|---|---|---|
| 출전 A 탭 → 출전 B 탭 | **자리 바꾸기**(위치 교대) | `SWAP_SOCCER_LINEUP_POSITIONS` (신규) | 없음 (GK 관여 시에도 `gkChange` 미기록 — 최종 `gk`만 키퍼) |
...
```
action: { type, matchIdx, aIdx, bIdx }
...
reducer:
  ...
  // 편집기 위치교대는 사후 정정 — gkChange 미기록. 최종 gk만 키퍼 집계.
  // (라이브 handleSwap의 gkChange는 유지 — 경기 중 실제 교대이므로 두 GK 집계 필요.)
  return { ...m, assignments: res.assignments, positionMap: res.positionMap,
           gk: res.gk, defenders }(그 경기만 교체한 soccerMatches)
```

- `swapFormationSlots`는 순수 헬퍼(기존) → 그대로 재사용.
- 스코어 재계산 불필요(교대는 골 이벤트 무변경).
- **gkChange 미기록**: 편집기 위치교대는 사후 정정이라 키퍼 교체(2인 집계)가 아님.
  라이브 `FormationRecorder.handleSwap`(경기 중 실제 교대)은 gkChange 유지.
...
- `SWAP_SOCCER_LINEUP_POSITIONS`는 **defenders를 positionMap에서 재계산**(클린시트 정합).
  **`gkChange` 미기록**: 편집기 위치교대는 사후 정정이라 키퍼 교체(2인 집계)가 아님 — 최종 `gk`만 키퍼(최종 결정, commit aa5cd7a).
  라이브 `FormationRecorder.handleSwap`(경기 중 실제 교대)은 gkChange 유지.
...
- GK 슬롯 관여 교대 → `gk` 갱신. **`gkChange` 미기록**(편집기=사후 정정, 최종 gk만 키퍼).
- 非GK 교대 → events 길이 불변.
...
- 출전끼리 탭-탭 → 자리 교대(이벤트 무변경, GK 슬롯이어도 gkChange 미기록 — 최종 gk만 키퍼).

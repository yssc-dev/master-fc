...
- Produces: 액션 `{ type: 'SWAP_SOCCER_LINEUP_POSITIONS', matchIdx, aIdx, bIdx }` → 해당 경기의 두 슬롯 위치 교대, `defenders` 재계산. **`gkChange` 미기록**: 편집기 위치교대는 사후 정정이라 키퍼 교체(2인 집계)가 아님 — 최종 `gk`만 키퍼(최종 결정, commit aa5cd7a). 라이브 `handleSwap`(경기 중 실제 교대)은 gkChange 유지.
...
describe('gameReducer — SWAP_SOCCER_LINEUP_POSITIONS', () => {
  ...
  it('GK 슬롯 교대: gk 갱신, gkChange 미추가(편집기=정정)', () => {
    // 최종 결정(commit aa5cd7a): 편집기 SWAP은 gkChange 미기록 — 최종 gk만 키퍼.
...
```js
    ...
    // role이 바뀌면 defenders 재계산(클린시트 정합).
    // gkChange 미기록: 편집기 위치교대는 사후 정정 — 최종 gk만 키퍼(최종 결정, commit aa5cd7a).
    // (라이브 FormationRecorder.handleSwap은 gkChange 유지 — 경기 중 실제 교대이므로 다름.)
    // 전제: 편집기 진입 시 레거시는 승격됨.
...
- gkChange 다운스트림 안전 → 편집기 SWAP은 gkChange 미기록(최종 결정, commit aa5cd7a). 라이브 handleSwap은 gkChange 유지(기존 soccerScoring 필터 적용) → 회귀 없음
